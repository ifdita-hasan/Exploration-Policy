...
log_file = os.path.join(DATA_DIR, 'pretrain_policy_grid.log')
logging.basicConfig(
    level=logging.INFO,
...
# --- Count (state, action) pairs in expert dataset ---
from collections import defaultdict
import pickle
state_action_counts = defaultdict(int)
for s, a in expert_dataset:
    state_action_counts[(tuple(s), int(a))] += 1
...
if trained_policy:
    # Define save directory relative to the current script's location
    script_dir = os.path.dirname(os.path.abspath(__file__))
    save_dir = os.path.join(script_dir, "saved_models_exp")
    ...
    try:
        ...
        save_dir = None
    if save_dir:
        model_save_path = os.path.join(save_dir, "policy_il_trained_grid.pth")
